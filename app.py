...
def _serialize_toml(settings: dict, collective: dict, members: list[dict]) -> str:
    ...
    if label_overrides:
        lines.append("")
        lines.append("[collective.label_overrides]")
        for key, val in sorted(label_overrides.items()):
            if val.strip():
    ...
    for member in members:
        ...
        for fee in member.get("custom_fees", []):
            if fee.get("name"):  # Only serialize fees with a name
                ...
                lines.append(f"fee_type = {_q(fee.get('fee_type', 'yearly'))}")
                if fee.get("fee_type") == "per_kwh":
                    lines.append(f"basis = {_q(fee.get('basis', 'grid'))}")
                lines.append("")
...
def _init_state() -> None:
    ...
    coll.setdefault("label_overrides", {})
...
def _sidebar() -> None:
    ...
    with st.sidebar.expander(t["bill"], expanded=False):
        ...
        col_sy, col_sm = st.columns(2, gap="small")
        ...
        col_ey, col_em = st.columns(2, gap="small")
        ...
        st.divider()
        if st.button(t.get("customize_labels", "Customize bill labels"), use_container_width=True,
                      help=t.get("customize_labels_help", "")):
            st.session_state["_show_label_overrides_dialog"] = True
            st.rerun()

    # Label overrides dialog (outside expander to render properly)
    if st.session_state.get("_show_label_overrides_dialog"):
        _render_label_overrides_dialog()
        st.session_state["_show_label_overrides_dialog"] = False


# -- Customisable bill translation keys, grouped for the dialog ---------------

_LABEL_GROUPS: list[tuple[str, list[tuple[str, str]]]] = [
    ("label_group_header", [
    ...
    ("label_group_summary", [
        ...
        ("net", "Net"),
        ("saved_with_solar", "Saved with solar"),
        ("you_saved", "You saved …"),
    ...
    ("label_group_daily", [
        ...
        ("total_kwh", "Total kWh"),
        ("prod_kwh", "Prod. kWh"),
        ("local_currency", "Local (currency col)"),
        ("grid_currency", "Grid (currency col)"),
        ("subtotal", "Subtotal"),
    ]),
    ("label_group_fees", [
...
def _render_label_overrides_dialog() -> None:
    """Render a dialog that lets the user override every bill PDF label."""
    from src.translations import get_translations

    t = _gui_lang()
    c = st.session_state["collective"]
    lang = c.get("language", "en")
    defaults = get_translations(lang)

    overrides: dict[str, str] = c.get("label_overrides", {})

    @st.dialog(t.get("customize_labels", "Customize bill labels"), width="large")
    def _labels_dialog():
    ...
    def _custom_fees_dialog():
        ...
        def _fees_editor():
            ...
            if pending:
                ...
                if action_type == "add":
                    fees.append({"name": "", "value": 0.0, "fee_type": "yearly", "basis": "grid"})
            ...
            if fees:
                ...
                def _fee_label(fee: dict, idx: int) -> str:
                    ...
                    ft = fee.get("fee_type", "yearly")
                    if ft == "per_kwh":
                        basis_lbl = t.get("fee_basis_grid", "Grid") if fee.get("basis") == "grid" else t.get("fee_basis_local", "Local")
                        return f"{name} ({val:.4f} CHF/kWh {basis_lbl})"
                ...
                # Fee type options
                _fee_types = ["yearly", "per_kwh"]
                _fee_type_labels = [t.get("fee_type_yearly", "Yearly"), t.get("fee_type_per_kwh", "Per kWh")]
                _basis_keys = ["grid", "local"]
                _basis_labels = [t.get("fee_basis_grid", "Grid (BKW)"), t.get("fee_basis_local", "Local (Solar)")]

                # Editable fee details
                for i, fee in enumerate(fees):
                    current_type = fee.get("fee_type", "yearly")
                    is_per_kwh = current_type == "per_kwh"

                    # Layout: Name | Value | Type | Basis (if per_kwh) | Remove
                    if is_per_kwh:
                    ...
                    # Fee value
                    fmt = "%.4f" if is_per_kwh else "%.2f"
                    step = 0.0001 if is_per_kwh else 0.1
                    ...
                    # Fee type dropdown
                    type_idx = _fee_types.index(current_type) if current_type in _fee_types else 0
                    selected_type_label = col3.selectbox(
                        t.get("fee_type", "Type"),
                        _fee_type_labels,
                        index=type_idx,
                        key=f"fee_type_{selected_idx}_{i}",
                        label_visibility="collapsed"
                    )
                    fee["fee_type"] = _fee_types[_fee_type_labels.index(selected_type_label)]

                    # Basis dropdown (for per_kwh) or unit label (for yearly)
                    if fee["fee_type"] == "per_kwh":
                        current_basis = fee.get("basis", "grid")
                        basis_idx = _basis_keys.index(current_basis) if current_basis in _basis_keys else 0
                        selected_basis_label = col4.selectbox(
                            t.get("fee_basis", "Basis"),
                            _basis_labels,
                            index=basis_idx,
                            key=f"fee_basis_{selected_idx}_{i}",
                            label_visibility="collapsed"
                        )
                        fee["basis"] = _basis_keys[_basis_labels.index(selected_basis_label)]
                    else:
                        col4.markdown(f"<div style='margin-top: -8px;'>{t.get('fee_unit_yearly', 'CHF / year')}</div>", unsafe_allow_html=True)

                    # Remove button
                    if col5.button("✕", key=f"fee_remove_{selected_idx}_{i}", help=t["remove_fee"], use_container_width=True):
